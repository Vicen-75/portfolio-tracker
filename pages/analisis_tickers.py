...
g6.metric("Div. Yield",      fmt_pct(div_y))
...
if is_etf:
    ...
    # Gráfico de gauge — precio actual vs rango de analistas
    ...
    if not recs.empty:
        latest = recs.iloc[0] if "period" in recs.columns else recs.iloc[0]
        try:
    ...
    if upside is not None:
        ...
        analyst_score = 1  # neutral si no hay datos
...
t1.metric("Posición 52 semanas",
          f"{pos_52w:.0f}%",
          help=f"Mín: ${low52:.2f}  ·  Máx: ${high52:.2f}")
...
fig_tech.add_trace(go.Scatter(
    ...
    line=dict(color="#534AB7", width=1.5), fill="tonexty",
...
tech_signals = []
if rsi_now < 30:
    tech_signals.append(2)   # sobrevendido = oportunidad de entrada
elif rsi_now < 45:
    tech_signals.append(2)   # zona de compra
elif rsi_now < 60:
    tech_signals.append(1)   # neutral
elif rsi_now < 70:
    tech_signals.append(1)   # precaución
else:
    tech_signals.append(0)   # sobrecomprado = esperar

if ma200_now:
    tech_signals.append(2 if price_now > ma200_now else 0)

if ma50_now and ma200_now:
    tech_signals.append(2 if ma50_now > ma200_now else 0)  # golden/death cross
...
# Puntuaciones: 0 = negativo · 1 = neutral · 2 = positivo
scores = {}
if not is_etf and fund_score is not None:
...
for col, (bloque, score) in zip([v1, v2, v3], scores.items()):
    ...
    col.metric(bloque, LABELS[score_int])

# Resumen rápido para tomar acción
